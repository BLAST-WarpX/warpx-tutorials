...
4. Restore the baseline input and run on GPU. What is the speedup with respect to the CPU runs?
...
`two_stream_instability/`, run `cd ..` first. The startup script prepares the wakefield files
...
Try running on CPU and GPU. How does the performance differ?
